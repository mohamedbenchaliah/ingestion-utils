...
from ingestor.quality._constraints._Numbers import _Number
...
class _StatColumn(_Number, ABC):
    ...
    def filter_success(self, data_frame: DataFrame) -> DataFrame:
        return data_frame.filter(
            f"{self.constraint_column_name} >= {self.lower_bound} "
            f"AND {self.constraint_column_name} <= {self.upper_bound}"
        )

    def filter_failure(self, data_frame: DataFrame) -> DataFrame:
        return data_frame.filter(
            f"{self.constraint_column_name} < {self.lower_bound} "
            f"OR {self.constraint_column_name} > {self.upper_bound}"
        )
...
class _MedianColumn(_StatColumn):
    def prepare_df_for_check(self, data_frame: DataFrame) -> DataFrame:
        ...
        average: DataFrame = data_frame.groupby().agg(median.alias(self.constraint_column_name))
