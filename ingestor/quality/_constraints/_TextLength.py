        ...
        if not parent_validation_result[0]:
            ...
            if column_type != "string":
                ...
                return (
                    ...
                    f"Upper bound ({self.upper_bound}) cannot be lower than lower bound ({self.lower_bound}).",
                )

    def filter_success(self, data_frame: DataFrame) -> DataFrame:
        return data_frame.filter(
            f"LENGTH({self.column_name}) >= {self.lower_bound} " f"AND LENGTH({self.column_name}) <= {self.upper_bound}"
        )

    def filter_failure(self, data_frame: DataFrame) -> DataFrame:
        return data_frame.filter(
            f"LENGTH({self.column_name}) < {self.lower_bound} " f"OR LENGTH({self.column_name}) > {self.upper_bound}"
        )
